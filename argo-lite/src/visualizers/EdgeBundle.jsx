...
@observer
class EdgeBundle extends React.Component {
  componentDidMount() {}
  ...
  setCircleColor = (node) => {
    ...
    if (
      ...
      node.id === appState.graph.currentlyHovered.id
    ) {
      return "red";
    } else {
      return "blue";
  ...
  setEdgePathOption = (edge) => {
    if (
      ...
      return {
        color: edge.data.withinFamily
          ? appState.graph.edges.color
          : appState.graph.edges.crossColor,
        weight: "1",
        opacity: "1",
      };
    }
          ...
          {appState.graph.rawGraph.edges[0].fromlocLatY !== undefined &&
            ...
            appState.graph.frame.getEdgeList().map((edge, i) => {
              ...
              return (
                <Polyline
                  key={i}
                  pathOptions={this.setEdgePathOption(edge)}
